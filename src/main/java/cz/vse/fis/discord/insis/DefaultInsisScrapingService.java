...
public class DefaultInsisScrapingService implements InsisScrapingService {

    private final int parallelism;

    private final ReactorHttpClient client;

    private final Logger logger = LoggerFactory.getLogger(DefaultInsisScrapingService.class);
    ...
    public Mono<Set<InsisSubject>> scrapeAvailableSubjects() {
        logger.info("Scraping the list of all subjects among all study programmes.");
                        ...
                        first.classNames()
                            ...
                            .map(className -> document.select("." + className)
                                ...
                                .map(row -> {
                                    ...
                                    return new InsisSubject(code.toLowerCase(), name);
                                })
                            )
                            .orElse(Stream.empty())
                    )
